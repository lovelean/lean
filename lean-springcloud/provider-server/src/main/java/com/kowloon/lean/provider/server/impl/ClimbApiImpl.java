...
public class ClimbApiImpl implements IClimbApi {
	...
	public Map<String,String> front(@Param("step") String step) {
		log.info("[provider-server]服务[ClimbApi]的[front]方法被调用，参数step值为：{}", step);
		Map<String,String> map = new HashMap<String, String>();
		map.put("info", "[provider-server]服务[ClimbApi]调用,往前爬了"+step+"步!");
		return map;
	}

	@Override
	public String back(@Param("step") String step) {
		log.info("[provider-server]服务[ClimbApi]的[back]方法被调用，参数step值为：{}", step);
		return "[provider-server]服务[ClimbApi]调用,往后爬了"+step+"步!";
	}

	@Override
	public String left(@Param("step") String step) {
		log.info("[provider-server]服务[ClimbApi]的[left]方法被调用，参数step值为：{}", step);
		return "[provider-server]服务[ClimbApi]调用,往左爬了"+step+"步!";
	}

	@Override
	public String right(@Param("step") String step) {
		log.info("[provider-server]服务[ClimbApi]的[right]方法被调用，参数step值为：{}", step);
		return "[provider-server]服务[ClimbApi]调用,往右爬了"+step+"步!";
	}
}
